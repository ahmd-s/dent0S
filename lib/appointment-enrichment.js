...
export async function enrichAppointments(db, clinicId, appointments) {
  ...
  const [pts, docs, chairs, visits, labCases, invoices] = await Promise.all([
    pids.length
      ? db.collection('patients').find(
        { id: { $in: pids }, clinic_id: clinicId },
        { projection: { _id: 0, id: 1, name: 1, phone: 1, gender: 1, date_of_birth: 1, photo_url: 1, total_visits: 1, is_vip: 1 } }
      ).toArray()
      : [],
    dids.length
      ? db.collection('profiles').find(
        { id: { $in: dids }, clinic_id: clinicId },
        { projection: { _id: 0, id: 1, full_name: 1 } }
      ).toArray()
      : [],
    cids.length
      ? db.collection('clinic_chairs').find(
        { id: { $in: cids }, clinic_id: clinicId },
        { projection: { _id: 0, id: 1, name: 1 } }
      ).toArray()
      : [],
    db.collection('visits').find(
      { clinic_id: clinicId, appointment_id: { $in: aids } },
      { projection: { _id: 0, id: 1, appointment_id: 1 } }
    ).toArray(),
    pids.length
    ...
    pids.length
      ? db.collection('invoices').find(
        ...
        { projection: { _id: 0, patient_id: 1, total_amount: 1, amount_paid: 1 } }
      ).toArray()
      : [],
  ])
  ...
  return appointments.map(a => {
    ...
    const age = pt?.date_of_birth
      ? Math.floor((Date.now() - new Date(pt.date_of_birth).getTime()) / (365.25 * 24 * 3600000))
      : null
    return {
      ...clean(a),
      ...
      patient_gender: pt?.gender || null,
      patient_age: age,
      patient_photo_url: pt?.photo_url || null,
      patient_total_visits: pt?.total_visits || 0,
      is_vip: pt?.is_vip || false,
      is_follow_up: a.appointment_type === 'follow_up',
      is_walk_in: a.is_walk_in || a.booked_via === 'walk_in',
      is_emergency: a.priority === 'emergency' || a.appointment_type === 'emergency',
      ...
      payment_status: a.patient_id && balanceByPatient[a.patient_id] > 0 ? 'pending' : 'clear',
    }
  })
}
