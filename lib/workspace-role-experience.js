...
export function normalizeNavOrder(order) {
  ...
  const next = base.filter(k => DEFAULT_NAV_ORDER.includes(k))

  // Patients is a core clinic surface. Dropped legacy keys (e.g. visits) used
  // to slide Lab Cases above it; keep Patients above Labs in every role.
  const patientsIdx = next.indexOf('patients')
  const labsIdx = next.indexOf('labs')
  if (patientsIdx !== -1 && labsIdx !== -1 && labsIdx < patientsIdx) {
    next.splice(patientsIdx, 1)
    next.splice(labsIdx, 0, 'patients')
  }
  return next
}
