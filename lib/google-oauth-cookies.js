...
export function setOAuthStateCookie(state) {
  cookies().set(OAUTH_STATE_COOKIE, state, oauthCookieOptions(60 * 10))
}

export function consumeOAuthStateCookie() {
  const jar = cookies()
  const state = jar.get(OAUTH_STATE_COOKIE)?.value
  jar.set(OAUTH_STATE_COOKIE, '', { ...oauthCookieOptions(0), maxAge: 0 })
  return state || null
}

export function buildGoogleSignupPendingToken({ email, full_name, google_sub }) {
  return signToken(
    {
...
export function setGoogleSignupPendingCookie({ email, full_name, google_sub }) {
  const token = buildGoogleSignupPendingToken({ email, full_name, google_sub })
  cookies().set(GOOGLE_SIGNUP_PENDING_COOKIE, token, oauthCookieOptions(60 * 15))
}
...
export function clearGoogleSignupPendingCookie() {
  cookies().set(GOOGLE_SIGNUP_PENDING_COOKIE, '', { ...oauthCookieOptions(0), maxAge: 0 })
}
...
export function setGooglePlatformAdminPendingCookie(pendingToken, setupRequired) {
  ...
  cookies().set(GOOGLE_PA_PENDING_COOKIE, value, oauthCookieOptions(PA_PENDING_MAX_AGE))
}

export function consumeGooglePlatformAdminPendingCookie() {
  const raw = cookies().get(GOOGLE_PA_PENDING_COOKIE)?.value
  cookies().set(GOOGLE_PA_PENDING_COOKIE, '', { ...oauthCookieOptions(0), maxAge: 0 })
  if (!raw) return null
  try {
