...
let platformTemplatesCache = { at: 0, data: null }
const PLATFORM_TEMPLATES_TTL_MS = 60_000

export async function getPlatformTemplates(db) {
  if (platformTemplatesCache.data && Date.now() - platformTemplatesCache.at < PLATFORM_TEMPLATES_TTL_MS) {
    return platformTemplatesCache.data
  }
  const settings = await db.collection('platform_settings').findOne({ _type: 'global' })
  const stored = settings?.[PLATFORM_TEMPLATES_KEY]
  const merged = mergeRoleTemplates(stored, DEFAULT_ROLE_TEMPLATES)
  platformTemplatesCache = { at: Date.now(), data: merged }
  return merged
}
...
async function savePlatformTemplatesRaw(db, templates) {
  ...
  platformTemplatesCache = { at: 0, data: null }
...
export async function getWorkspaceForSession(db, clinicId) {
  if (!clinicId) {
  ...
  return createDefaultWorkspace(db, clinicId)
}
