...
export async function doctorPatientIds(db, clinicId, doctorId) {
  const filter = { clinic_id: clinicId, doctor_id: doctorId, patient_id: { $type: 'string' } }
  const [fromAppts, fromVisits] = await Promise.all([
    db.collection('appointments').distinct('patient_id', filter),
    db.collection('visits').distinct('patient_id', filter),
  ])
  return [...new Set([...fromAppts, ...fromVisits].filter(Boolean))]
}
