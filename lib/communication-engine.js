...
export async function getCommunicationDashboard(db, clinicId) {
  ...
  const dobSuffix = `-${today.slice(5)}`

  const [msgFacet, reviewRequests, birthdays, lastMessages, scheduledMessages] = await Promise.all([
    db.collection('communication_messages').aggregate([
      { $match: { clinic_id: clinicId } },
      {
        $facet: {
          todays: [
            { $match: { created_at: { $gte: startOfDay, $lte: endOfDay } } },
            {
              $group: {
                ...
                delivered: {
                  $sum: {
                    $cond: [{ $in: ['$status', [MESSAGE_STATUS.SENT, MESSAGE_STATUS.DELIVERED]] }, 1, 0],
                  },
                },
                failed: {
                ...
                payment: {
                  $sum: { $cond: [{ $eq: ['$type', MESSAGE_TYPES.PAYMENT_REMINDER] }, 1, 0] },
                },
                lab: {
                  $sum: { $cond: [{ $eq: ['$type', MESSAGE_TYPES.LAB_UPDATE] }, 1, 0] },
                },
                appointment: {
                  $sum: { $cond: [{ $eq: ['$type', MESSAGE_TYPES.APPOINTMENT_REMINDER] }, 1, 0] },
                },
              },
            },
          ],
          ...
          pending: [
            { $match: { status: MESSAGE_STATUS.PENDING } },
            { $count: 'n' },
          ],
        },
      },
    ...
    db.collection('communication_messages')
      ...
      .toArray(),
    db.collection('communication_messages')
      .find({ clinic_id: clinicId, status: MESSAGE_STATUS.SCHEDULED })
      .sort({ scheduled_at: 1 })
      .limit(10)
      .project({ _id: 0, password_hash: 0 })
      .toArray(),
  ...
  const facet = msgFacet[0] || {}
  const todays = facet.todays?.[0] || {}

  const result = {
    ok: true,
    todays_reminders: todays.total || 0,
    scheduled_today: facet.scheduled_today?.[0]?.n || 0,
    delivered: todays.delivered || 0,
    failed: todays.failed || 0,
    pending: facet.pending?.[0]?.n || 0,
    review_requests: reviewRequests,
    birthdays,
    payment_reminders: todays.payment || 0,
    lab_notifications: todays.lab || 0,
    appointment_reminders: todays.appointment || 0,
