...
/**
 * High-frequency events that do not need a distinct row per keystroke /
 * drag. Clinical milestones (payments, visit complete, consent) are never
 * in this set — those stay one-row-per-occurrence for the audit trail.
 */
const MERGEABLE_EVENTS = new Set([
  ACTIVITY_EVENTS.QUEUE_POSITION_CHANGED,
  ACTIVITY_EVENTS.TOOTH_UPDATED,
  ACTIVITY_EVENTS.PATIENT_UPDATED,
  ACTIVITY_EVENTS.LAB_UPDATED,
  ACTIVITY_EVENTS.LAB_STATUS_UPDATED,
])
...
export async function logEvent(db, {
  ...
  try {
    if (MERGEABLE_EVENTS.has(event)) {
      const merged = await tryMergeRecent(db, {
        clinicId,
        event,
...
async function tryMergeRecent(db, {
  clinicId,
  event,
  actorId,
  patientId,
  ...
  if (metadata?.tooth != null) filter['metadata.tooth'] = metadata.tooth
  if (metadata?.tooth_id != null) filter['metadata.tooth_id'] = metadata.tooth_id
  if (metadata?.item_id != null) filter['metadata.item_id'] = metadata.item_id

  const result = await db.collection(COLLECTION).findOneAndUpdate(
    filter,
    {
      $inc: { 'metadata.repeat_count': 1 },
      $set: { 'metadata.last_repeated_at': new Date() },
    },
