...
describe('voice recording error mapping', () => {
  ...
  it('keeps already-mapped clinician messages and surfaces decommissioned Groq models', () => {
    assert.equal(
      mapVoiceProviderError('No speech was detected. Record again closer to the microphone.'),
      'No speech was detected. Record again closer to the microphone.',
    )
    assert.match(
      mapVoiceProviderError({
        message: 'Request failed with status code 400',
        response: { status: 400, data: { error: { message: 'The model `llama-3.3-70b-versatile` has been decommissioned.' } } },
      }),
      /AI model used for notes is unavailable/,
    )
  })
})
