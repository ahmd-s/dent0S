...
export async function generateClinicalSummary(db, profile, { patientId, force = false } = {}) {
  ...
  try {
    ...
    await logRequest(db, profile, AI_REQUEST_TYPES.CLINICAL_SUMMARY, { patient_id: patientId, tokens_estimated: Math.ceil(text.length / 4) })

    const result = { ok: true, summary: text, generated_at: generatedAt, type: 'suggest' }
...
export async function generateVoiceVisitSummary(db, profile, { audioBuffer, visitId, patientId } = {}) {
  ...
  try {
    ...
    return { ok: false, error: e.message || 'Voice processing failed' }
  }
}
...
export async function getCopilotSnapshot(db, profile, { patientId } = {}) {
  ...
  const { patient, visits, prescriptions, balance, labCases, clinical, appointments } = ctx
...
export async function getAIDashboard(db, clinicId) {
  ...
  const recentRequests = await db.collection('ai_requests').find({ clinic_id: clinicId }).sort({ created_at: -1 }).limit(10).toArray()
