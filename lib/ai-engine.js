...
export async function generateVoiceVisitSummary(db, profile, { audioBuffer, visitId, patientId, mimeType, filename } = {}) {
  ...
  try {
    const extractionPrompt = `${SAFETY_PREFIX}\nExtract structured visit data from transcript. Return JSON only with: chief_complaint, clinical_notes, diagnosis, treatment_done, treatment_plan, prescriptions (array), followup_instructions, soap_notes (subjective/objective/assessment/plan), next_visit_recommended, next_visit_notes, next_visit_date (YYYY-MM-DD format or null). CRITICAL: Extract text VERBATIM from the transcript. Do NOT rephrase, do NOT add medical terminology not present in the transcript, do NOT expand or elaborate. Keep the extraction minimal and faithful to what was actually said. For next_visit_date, extract dates in YYYY-MM-DD format. Assume current year if not specified. Convert Indian date formats (DD/MM/YYYY, DD-MM-YYYY) to YYYY-MM-DD.`
