...
import { filenameForAudioMime, groqMimeForAudio } from '@/lib/voice-audio'
...
/** Groq retired llama-3.3-70b-versatile on 16 Aug 2026. Override with GROQ_CHAT_MODEL if needed. */
const GROQ_CHAT_MODEL = (process.env.GROQ_CHAT_MODEL || 'openai/gpt-oss-120b').trim()
const GROQ_WHISPER_MODEL = (process.env.GROQ_WHISPER_MODEL || 'whisper-large-v3').trim()

export function groqChatRequestBody(messages, maxTokens = 600, model = GROQ_CHAT_MODEL) {
  const payload = { model, messages }
  // GPT-OSS on Groq is a reasoning model: it rejects temperature/max_tokens.
...
export function extractGroqChatContent(data) {
...
async function groqChat(messages, maxTokens = 600) {
  ...
  const response = await axios.post(
    ...
    groqChatRequestBody(messages, maxTokens),
    {
  ...
  return extractGroqChatContent(response.data)
}
...
function fallbackVoiceExtraction(transcript, raw = '') {
  const text = String(transcript || '').trim()
  return {
    clinical_notes: String(raw || text).trim() || text,
    chief_complaint: text.slice(0, 280),
  }
}

function voiceVisitResult(transcript, extracted) {
  const fields = extracted && typeof extracted === 'object' ? extracted : fallbackVoiceExtraction(transcript)
  return {
    ok: true,
    transcript,
    extracted: fields,
    soap_notes: fields.soap_notes || null,
    visit_draft: {
      chief_complaint: fields.chief_complaint || '',
      clinical_notes: fields.clinical_notes || '',
      diagnosis: fields.diagnosis || '',
      treatment_done: fields.treatment_done || '',
      treatment_plan: fields.treatment_plan || '',
      prescriptions: fields.prescriptions || [],
      followup_instructions: fields.followup_instructions || '',
    },
    type: 'draft',
...
async function transcribeAudioWithGroq({ key, audioBuffer, mimeType, filename }) {
  ...
  const file = typeof File === 'function'
    ? new File([bytes], groqName, { type: groqMime })
    : new Blob([bytes], { type: groqMime })
  form.append('file', file, groqName)
  ...
  try {
    response = await fetch('https://api.groq.com/openai/v1/audio/transcriptions', {
      ...
      body: form,
      signal: AbortSignal.timeout(GROQ_TIMEOUT_MS * 2),
    })
  } catch (e) {
  ...
  if (!response.ok) {
    const err = new Error(data?.error?.message || `Groq transcription failed (${response.status})`)
...
export async function generateVoiceVisitSummary(db, profile, { audioBuffer, visitId, patientId, mimeType, filename } = {}) {
  ...
  let transcript
  try {
    transcript = await transcribeAudioWithGroq({ key, audioBuffer, mimeType, filename })
  } catch (e) {
    return { ok: false, error: e }
  }

  if (!transcript) {
    return { ok: false, error: 'No speech was detected. Record again closer to the microphone.' }
  }

  let extracted = fallbackVoiceExtraction(transcript)
  try {
    const extractionPrompt = `${SAFETY_PREFIX}\nExtract structured visit data from transcript. Return ONLY valid JSON with these exact fields:\n\n{\n  "chief_complaint": "short sentence - what patient complains about (e.g., 'tooth pain', 'bleeding gums')",\n  "clinical_notes": "examination findings - what you observed clinically (e.g., 'swelling seen', 'mobility grade 2')",\n  "diagnosis": "diagnosis - the condition identified (e.g., 'chronic periodontitis', 'deep caries')",\n  "treatment_done": "treatment performed TODAY - look for phrases like 'treatment done today', 'we performed', 'procedure done', 'initiated', 'completed today' (e.g., 'scaling done', 'extraction completed')",\n  "treatment_plan": "plan for future visits - what needs to be done next (e.g., 'RCT in next visit', 'crown placement after 2 weeks')",\n  "prescriptions": [{"medicine_name": "", "dosage": "", "frequency": "", "duration": "", "instructions": ""}],\n  "next_visit_recommended": false,\n  "next_visit_notes": "",\n  "next_visit_date": null\n}\n\nCRITICAL INSTRUCTIONS:\n1. Put ONLY complaint-related text in chief_complaint\n2. Put ONLY examination/clinical findings in clinical_notes\n3. Put ONLY diagnosis in diagnosis\n4. Put ONLY what was done TODAY in treatment_done\n5. Put ONLY future plans in treatment_plan\n6. Extract text VERBATIM - do NOT rephrase, do NOT add medical terminology not present in the transcript\n7. Use empty string for missing text fields\n8. Use false for next_visit_recommended if not mentioned\n9. Use null for next_visit_date if no date mentioned\n10. Use empty array for prescriptions if none mentioned\n11. For next_visit_date, extract dates in YYYY-MM-DD format. Assume current year if not specified. Convert Indian date formats (DD/MM/YYYY, DD-MM-YYYY) to YYYY-MM-DD.\n12. next_visit_recommended should be true if follow up is mentioned\n\nTranscript:\n${transcript}`
    ...
    try {
      ...
      extracted = fallbackVoiceExtraction(transcript, raw)
    }
  } catch {
    // Whisper already succeeded. Do not fail the visit because the chat model is down.
    extracted = fallbackVoiceExtraction(transcript)
  }
  ...
  return voiceVisitResult(transcript, extracted)
}
