...
async function communicationUrgencyLite(db, clinicId, today) {
  ...
  const [row] = await db.collection('communication_messages').aggregate([
    { $match: { clinic_id: clinicId } },
    {
      $facet: {
        pending: [{ $match: { status: 'pending' } }, { $count: 'n' }],
        scheduledToday: [{
          $match: { status: 'scheduled', scheduled_at: { $gte: startOfDay, $lte: endOfDay } },
        }, { $count: 'n' }],
        failedToday: [{
          $match: { status: 'failed', created_at: { $gte: startOfDay, $lte: endOfDay } },
        }, { $count: 'n' }],
      },
    },
  ]).toArray()
  const pending = row?.pending?.[0]?.n || 0
  const scheduledToday = row?.scheduledToday?.[0]?.n || 0
  const failedToday = row?.failedToday?.[0]?.n || 0
  return {
    ok: true,
    pending,
...
async function computeDashboardPayload(db, profile, clinic, { mode, today, yest, timezone, timings, marks, t0 }) {
    ...
    db.collection('appointments').aggregate([
      {
        $match: {
          clinic_id: cid,
          appointment_date: { $in: [today, yest] },
          status: 'completed',
          ...doctorFilter,
        },
      },
      { $group: { _id: '$appointment_date', n: { $sum: 1 } } },
    ]).toArray(),
  ...
  const doneToday = completedRows.find(r => r._id === today)?.n || 0
  const doneYest = completedRows.find(r => r._id === yest)?.n || 0
  const revAgg = [{ sum: invoiceRow[0]?.revenue || 0 }]
  const pendAgg = [{ sum: invoiceRow[0]?.pending || 0 }]
  const followups = followupDocs.slice(0, 5)
  const fcount = followupDocs.length > 5
    ? await db.collection('patients').countDocuments(followupFilter)
    : followups.length
  mark?.('core_queries')
 ...
 *   core = queue, today's KPIs, follow-ups, lab counts (first paint)
 *   full = core + inventory/communication/analytics/AI/lab detail
 */
