...
/** Optional `db` lets scripts/tests inject a connection without mutating getDb. */
export async function setupIndexes(db) {
  ...
  // Dashboard: today queue + completed counts + doctor scope
  await db.collection('appointments').createIndex({ clinic_id: 1, appointment_date: 1, status: 1 })
  await db.collection('appointments').createIndex({ clinic_id: 1, appointment_date: 1, doctor_id: 1, appointment_time: 1 })
  ...
  // Dashboard pending-tasks: clinic + open workflow + step status
  await db.collection('visits').createIndex({ clinic_id: 1, workflow_status: 1, updated_at: -1 })
  await db.collection('visits').createIndex({ share_token: 1 }, { unique: true, sparse: true })
  ...
  // INVENTORY — dashboard lite metrics
  console.log('Creating indexes for inventory collections...')
  await db.collection('inventory_items').createIndex({ clinic_id: 1, is_active: 1 })
  await db.collection('inventory_items').createIndex({ clinic_id: 1, item_name: 1 })
  await db.collection('stock_movements').createIndex({ clinic_id: 1, created_at: -1 })
  await db.collection('stock_movements').createIndex({ clinic_id: 1, item_id: 1, created_at: -1 })
  await db.collection('inventory_purchases').createIndex({ clinic_id: 1, status: 1 })
  await db.collection('inventory_purchases').createIndex({ clinic_id: 1, received_at: -1 })
  await db.collection('inventory_batches').createIndex({ clinic_id: 1, expiry_date: 1 })
  ...
  // COMMUNICATION collections (Sprint 17)
  console.log('Creating indexes for communication collections...')
  await db.collection('communication_messages').createIndex({ clinic_id: 1, created_at: -1 })
  await db.collection('communication_messages').createIndex({ clinic_id: 1, patient_id: 1, created_at: -1 })
  await db.collection('communication_messages').createIndex({ clinic_id: 1, status: 1, scheduled_at: 1 })
  await db.collection('communication_messages').createIndex({ clinic_id: 1, type: 1, created_at: -1 })
  await db.collection('communication_messages').createIndex({ clinic_id: 1, appointment_id: 1 })
  await db.collection('communication_campaigns').createIndex({ clinic_id: 1, created_at: -1 })
  await db.collection('communication_campaigns').createIndex({ clinic_id: 1, status: 1 })
  await db.collection('communication_templates').createIndex({ clinic_id: 1 })
  await db.collection('communication_reviews').createIndex({ clinic_id: 1, status: 1 })
  await db.collection('communication_reviews').createIndex({ clinic_id: 1, patient_id: 1 })
  console.log('✓ Communication indexes created')

  // Provider-agnostic communication module
  console.log('Creating indexes for communication module collections...')
  await db.collection('messages').createIndex({ clinic_id: 1, created_at: -1 })
  await db.collection('messages').createIndex({ clinic_id: 1, status: 1, scheduled_at: 1 })
  await db.collection('messages').createIndex({ clinic_id: 1, patient_id: 1, created_at: -1 })
  await db.collection('messages').createIndex({ clinic_id: 1, type: 1, created_at: -1 })
  await db.collection('messages').createIndex(
    { clinic_id: 1, idempotency_key: 1 },
    { unique: true, partialFilterExpression: { idempotency_key: { $type: 'string' } } }
  )
  await db.collection('message_attempts').createIndex({ clinic_id: 1, message_id: 1, created_at: -1 })
  await db.collection('message_templates').createIndex({ clinic_id: 1, type: 1 })
  await db.collection('communication_preferences').createIndex({ clinic_id: 1, patient_id: 1 }, { unique: true, partialFilterExpression: { patient_id: { $type: 'string' } } })
  await db.collection('communication_preferences').createIndex({ clinic_id: 1, profile_id: 1 }, { unique: true, partialFilterExpression: { profile_id: { $type: 'string' } } })
  await db.collection('provider_configs').createIndex({ clinic_id: 1 }, { unique: true })
  await db.collection('communication_events').createIndex({ clinic_id: 1, message_id: 1, created_at: -1 })
  await db.collection('communication_events').createIndex({ clinic_id: 1, event_type: 1, created_at: -1 })
  console.log('✓ Communication module indexes created')

  // AI collections (Sprint 18)
  console.log('Creating indexes for AI collections...')
  await db.collection('ai_requests').createIndex({ clinic_id: 1, created_at: -1 })
  await db.collection('ai_requests').createIndex({ clinic_id: 1, type: 1, created_at: -1 })
  await db.collection('ai_requests').createIndex({ clinic_id: 1, patient_id: 1 })
  await db.collection('ai_requests').createIndex({ clinic_id: 1, user_id: 1, created_at: -1 })
  await db.collection('ai_transcripts').createIndex({ clinic_id: 1, created_at: -1 })
  await db.collection('ai_transcripts').createIndex({ clinic_id: 1, visit_id: 1 })
  await db.collection('ai_transcripts').createIndex({ clinic_id: 1, patient_id: 1 })
  console.log('✓ AI indexes created')

  // Sprint 19 — Observability, jobs, rate limits
  console.log('Creating indexes for Sprint 19 collections...')
  await db.collection('system_logs').createIndex({ created_at: -1 })
  await db.collection('system_logs').createIndex({ clinic_id: 1, created_at: -1 })
  await db.collection('system_logs').createIndex({ level: 1, created_at: -1 })
  await db.collection('system_logs').createIndex({ category: 1, created_at: -1 })
  await db.collection('background_jobs').createIndex({ status: 1, scheduled_at: 1 })
  await db.collection('background_jobs').createIndex({ clinic_id: 1, status: 1 })
  await db.collection('background_jobs').createIndex({ type: 1, created_at: -1 })
  await db.collection('api_rate_limits').createIndex({ key: 1 }, { unique: true })
  await db.collection('api_rate_limits').createIndex({ updated_at: 1 })
  console.log('✓ Sprint 19 indexes created')
}
