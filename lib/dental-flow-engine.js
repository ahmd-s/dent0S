...
/** Compute flow metrics for dashboard/reports. */
export async function computeFlowMetrics(db, clinicId, date, doctorFilter = {}) {
  ...
  const appts = await db.collection('appointments').find(filter).toArray()
  const chairs = await db.collection('clinic_chairs').find({ clinic_id: clinicId, is_active: { $ne: false } }).toArray()

  const now = new Date()
  const { computeQueueWaitStats } = await import('@/lib/flow-waiting-timer')
