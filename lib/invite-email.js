...
export async function sendPasswordResetEmail({ to, resetUrl }) {
  ...
  const subject = 'Reset your DentOS password'
  const html = `
    <p>We received a request to reset the password for your DentOS account.</p>
    <p><a href="${escapeAttr(resetUrl)}">Reset your password</a></p>
    <p>Or copy this link into your browser:<br/>${escapeHtml(resetUrl)}</p>
    <p>This link expires in 1 hour and can only be used once.</p>
    <p style="color:#64748b;font-size:12px;margin-top:24px">If you did not request a password reset, you can ignore this email.</p>
  `.trim()
  const r = await fetch('https://api.resend.com/emails', {
    method: 'POST',
    headers: { Authorization: `Bearer ${apiKey}`, 'Content-Type': 'application/json' },
  ...
  if (!r.ok) {
    const t = await r.text().catch(() => '')
    console.error('Resend password reset error:', r.status, t)
    return { sent: false, reason: 'resend_error', status: r.status }
  }
  return { sent: true }
}

export async function sendEmailVerificationEmail({ to, verifyUrl, clinicName }) {
  const apiKey = process.env.RESEND_API_KEY
  const from = process.env.RESEND_FROM_EMAIL
  if (!apiKey || !from) {
    return { sent: false, reason: 'missing_resend_config' }
  }
  const subject = `Verify your email for ${clinicName || 'DentOS'}`
  const html = `
    <p>Welcome to DentOS!</p>
    <p>Please verify your email address to activate your account for <strong>${escapeHtml(clinicName || 'your clinic')}</strong>.</p>
    <p><a href="${escapeAttr(verifyUrl)}">Verify your email</a></p>
    <p>Or copy this link into your browser:<br/>${escapeHtml(verifyUrl)}</p>
    <p>This link expires in 24 hours.</p>
    <p style="color:#64748b;font-size:12px;margin-top:24px">If you did not create a DentOS account, you can ignore this email.</p>
  `.trim()
  const r = await fetch('https://api.resend.com/emails', {
    method: 'POST',
    headers: { Authorization: `Bearer ${apiKey}`, 'Content-Type': 'application/json' },
  ...
  if (!r.ok) {
    const t = await r.text().catch(() => '')
    console.error('Resend email verification error:', r.status, t)
    return { sent: false, reason: 'resend_error', status: r.status }
  }
  return { sent: true }
}
