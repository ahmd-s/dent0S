...
export async function processMessage(db, profile, messageId) {
  ...
  let failureReason = providerResult.detail?.reason
    || (providerResult.outcome === PROVIDER_OUTCOMES.REJECTED ? 'rejected' : null)
  ...
  if (providerResult.detail?.provider_message_id) {
    update.provider_message_id = providerResult.detail.provider_message_id
  }

  if (nextStatus === MESSAGE_STATUSES.ACCEPTED) {
    update.manual_send_required = false
  ...
  if (nextStatus === MESSAGE_STATUSES.RETRY_SCHEDULED) {
    ...
    if (providerResult.detail?.reason) update.failure_reason = providerResult.detail.reason
  }
  ...
  } else if (nextStatus === MESSAGE_STATUSES.ACCEPTED) {
    await recordCommunicationEvent(db, profile, updated, 'message_accepted', {
...
export async function listMessages(db, clinicId, opts = {}) {
  ...
  if (filter === 'sent') query.status = { $in: [MESSAGE_STATUSES.SENT, MESSAGE_STATUSES.ACCEPTED] }
  if (filter === 'accepted') query.status = MESSAGE_STATUSES.ACCEPTED
