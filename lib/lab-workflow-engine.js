...
export async function computeLabMetrics(db, clinicId, filters = {}) {
  ...
  return computeLabMetricsFromCases(cases)
}

/** Dashboard-friendly lab metrics — projected fields only, no unbounded document bloat. */
export async function computeLabMetricsLite(db, clinicId) {
  const cases = await db.collection('lab_cases').find(
    { clinic_id: clinicId },
    {
      projection: {
        _id: 0,
...
function computeLabMetricsFromCases(cases) {
