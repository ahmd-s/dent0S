...
async function loadCoreData(db, clinicId, range) {
  ...
  const [
    ...
    db.collection('activity_events').find({
      clinic_id: clinicId,
  ...
  return {
    invoices, prevInvoices, patients, appointments, visits, profiles, activityEvents, labCases,
...
export async function getAppointmentAnalytics(db, clinicId, opts = {}) {
  ...
  const { appointments, activityEvents } = data
  ...
  const waitEvents = activityEvents.filter(e =>
    [ACTIVITY_EVENTS.PATIENT_CHECKED_IN, ACTIVITY_EVENTS.APPOINTMENT_CHECKED_IN].includes(e.event)
  )
  const durationEvents = activityEvents.filter(e => e.metadata?.duration_minutes)
...
export async function getInventoryAnalytics(db, clinicId) {
  ...
  const cached = getCached(key)
  if (cached) return cached
  const metrics = await computeInventoryMetrics(db, clinicId)
  const result = {
    ...metrics,
...
export async function getLabAnalytics(db, clinicId) {
  ...
  const cached = getCached(key)
  if (cached) return cached
  const metrics = await computeLabMetrics(db, clinicId)
