  ...
  try {
    ...
    switch (job.type) {
      ...
      case JOB_TYPES.COMMUNICATION_QUEUE: {
        const { processScheduledMessages } = await import('@/lib/communication-engine')
        result = await processScheduledMessages(db, job.clinic_id || null)
        break
      }
...
/** Process all pending jobs (cron entry point). */
export async function processPendingJobs(db, { limit = 20 } = {}) {
  const jobs = await getPendingJobs(db, { limit })
  const results = []
  for (const job of jobs) {
    results.push({ id: job.id, type: job.type, ...(await processJob(db, job)) })
  }
...
export async function scheduleRecurringJobs(db) {
  ...
  for (const spec of recurring) {
    const existing = await db.collection(COLLECTION).findOne({
      type: spec.type,
      status: { $in: [JOB_STATUS.PENDING, JOB_STATUS.RUNNING, JOB_STATUS.RETRY] },
    })
    if (!existing) {
      const job = await enqueueJob(db, { type: spec.type, priority: spec.priority })
      created.push(job.id)
    }
  }
  return { created }
