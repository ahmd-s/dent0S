...
export async function GET(request, { params }) {
  try {
    ...
    const v = await db.collection('visits').findOne({ id: params.id, clinic_id: cid })
...
export async function PUT(request, { params }) {
  try {
    ...
    if (Array.isArray(b.prescriptions)) {
      ...
      if (valid.length) {
        await db.collection('prescriptions').insertMany(valid.map(r => ({ id: uuidv4(), clinic_id: cid, visit_id: params.id, medicine_name: r.medicine_name, dosage: r.dosage||'', frequency: r.frequency||'', duration: r.duration||'', instructions: r.instructions||'', created_at: new Date() })))
      }
    }
    // upsert invoice items + invoice (draft mode)
    const visit = await db.collection('visits').findOne({ id: params.id, clinic_id: cid })
    let invoiceId = null
    if (Array.isArray(b.invoice_items) || b.invoice) {
      const existing = await db.collection('invoices').findOne({ visit_id: params.id, clinic_id: cid })
