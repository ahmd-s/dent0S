    ...
    <div className="min-h-screen bg-[#F8FAFC]">
      <div className="max-w-xl mx-auto px-4 py-12">
        <div className="bg-white rounded-2xl border border-border p-8 text-center">
          ...
          {info && (
            <div className="mt-6 p-5 bg-[#F8FAFC] rounded-lg border border-border text-left space-y-3">
              <div className="flex items-center gap-3 text-sm"><Calendar className="w-4 h-4 text-[#0D9488]"/><span>{fmtFull(info.date)}</span></div>
              <div className="flex items-center gap-3 text-sm"><Clock className="w-4 h-4 text-[#0D9488]"/><span>{info.time}</span></div>
              {info.doctor_name && <div className="flex items-center gap-3 text-sm"><span className="w-4 h-4 inline-flex items-center justify-center text-[#0D9488] font-bold">👨‍⚕️</span><span>Dr. {info.doctor_name}</span></div>}
