...
function App() {
  ...
  const [f, setF] = useState({ patient_id:'', appointment_date: todayIso(), appointment_time:'10:00 AM', appointment_type:'new_patient', chief_complaint:'', notes:'' })

  const load = async () => {
    const r = await fetch(`/api/appointments?date=${date}`)
    const d = await r.json(); setList(d.appointments || [])
  }
  useEffect(() => { load() }, [date])
  ...
  const submit = async e => {
    ...
    setLoading(false)
    if (r.ok) { toast.success('Appointment booked'); setOpen(false); load() }
  ...
  return (
    <div className="p-8 max-w-7xl">
      <div className="flex items-center justify-between">
        <div><h1 className="text-2xl font-bold text-[#0F172A]">Appointments</h1><p className="text-muted-foreground text-sm mt-1">{fmtDate(date)} · {list.length} scheduled</p></div>
            ...
            <DialogContent className="max-w-lg">
              ...
              <form onSubmit={submit} className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div className="space-y-1.5"><Label>Time</Label><Input value={f.appointment_time} onChange={e=>setF({...f,appointment_time:e.target.value})} placeholder="10:00 AM"/></div>
                </div>
                <div className="space-y-1.5"><Label>Type</Label>
                  <Select value={f.appointment_type} onValueChange={v=>setF({...f,appointment_type:v})}><SelectTrigger><SelectValue/></SelectTrigger>
                    <SelectContent>{TYPES.map(t=><SelectItem key={t} value={t} className="capitalize">{t.replace('_',' ')}</SelectItem>)}</SelectContent></Select>
                </div>
        ...
        {list.map(a => (
          <div key={a.id} className="flex items-center gap-4 px-6 py-4 border-b border-border last:border-0">
            ...
            <div className="flex-1">
              <div className="font-medium">{a.patient_name || a.patient_name_temp}</div>
              <div className="text-xs text-muted-foreground capitalize">{a.appointment_type?.replace('_',' ')} · {a.chief_complaint || 'No complaint'}</div>
            </div>
